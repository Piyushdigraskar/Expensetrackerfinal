function saveToLocal(event) {
    ...
    axios.post("http://localhost:3000/expense/add-user", expenseDetails, { headers: { "Authorization": token } })
        .then((respone) => {
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    axios.get(`http://localhost:3000/expense/get-user?page=${page}`, { headers: { "Authorization": token } })
        .then((respone) => {
...
function deleteExpense(event, expenseid) {
    const token = localStorage.getItem('token');
    axios.delete(`http://localhost:3000/expense/delete-user/${expenseid}`, { headers: { "Authorization": token } }).then(() => {
